...
| `in_legend` | boolean or string | `true` | v1.2.0 | If set to `raw`, it would display the latest raw state of the entity in the legend value, aimimg to compare offset-series 'now'. 'Similar as in_header, 'before_now` would display the value just before the current time and `after_now` would also display the value just after the current time (note: this is a gap from the original implementation) (Introduced in v1.2.0) |
