...
**Disclaimer**: 
I am not a seasoned developer and the card itself is quite technical as well, which adds challenges to my lack of development skills. As a result I will still try spending time to fix issues that are reasonable i.e. major and/or no workarounds found. Added to that I asked to add a few PR/issues noted from the source/RomRider and **I can accept PR's if they have been thoroughly tested**
...
| `float_precision` | number | `1` | v1.3.0 | The precision used to display data in the legend and the tooltip. It doesn't impact how the data is displayed on the graph. It adds trailing zero's until the configured number is reached (intro: 1.3.0)|
...
| `in_legend` | boolean or string | `true` | v1.2.0 | If set to `raw`, it would display the latest raw state of the entity in the legend value, aimimg to allow to compare offset-series with now(). Similar as in_header, `before_now` would display the value just before the current time, as well as `after_now` (note: this is a gap from the original implementation, and needs a fix/PR) (Introduced in v1.2.0) |
